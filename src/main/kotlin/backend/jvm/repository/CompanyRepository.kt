...
interface CompanyRepository : JpaRepository<Company, Int>{
    ...
    @Query(value = "from service s where s.cid = :id")
...
*/
    @Query(value = "from appointment a where a.sid = (select id from schedule s where s.comp_id = :id)")
    fun getAllAppointments(@Param("id") id: Int): List<Appointment>

    @Query(value = "from appointment a where a.sid = (select id from schedule s where s.comp_id = :id) and a.app_date = :date and a.app_hour = :time")
    fun getAppointment(@Param("id") id: Int, @Param("date") date: Date, @Param("hour") time: Time): Appointment

    @Query(value = "from sch_day d where d.sid = (select id from schedule s where s.comp_id = :id)")
    fun getOpenDays(@Param("id") id: Int): List <Day>

    @Query(value = "from vacation v where v.sid = (select id from schedule s where s.comp_id = :id)")
