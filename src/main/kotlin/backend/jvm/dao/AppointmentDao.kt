...
interface AppointmentDao: JpaRepository<AppointmentEntity, Int>{

    @Query(value = "select * from appointment where schedule_id = (select id from schedule s where s.company_id = :id)", nativeQuery = true)
    fun getAllOnGoingAppointments (@Param("id")id:Int): List<AppointmentEntity>

    @Query(value = "select * from appointment a where a.schedule_id=:sid and a.app_date=:date and a.app_hour=:hour", nativeQuery = true)
    fun getAppointmentByDateAndHour (@Param("sid")id:Int, @Param("date")date:Date, @Param("hour")hour:Time): List<AppointmentEntity>
    ...
    fun getAppointmentByUserDB(@Param("id") id: Int): List<AppointmentEntity>

    @Query(value = "select * from APPOINTMENT where APPOINTMENT.schedule_id = :schedule ", nativeQuery = true)
    fun getAppointmentsBySchedule(@Param("schedule") schedule: Int):List<AppointmentEntity>
