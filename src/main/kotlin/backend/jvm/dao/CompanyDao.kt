...
interface CompanyDao : JpaRepository<CompanyEntity, Int>{
    ...
    @Query(value = "select company.id from company inner join schedule s on company.id = s.company_id and s.id = :schedule", nativeQuery = true)
    fun getCompanyBySchedule(@Param("schedule") schedule: Int):Int?
